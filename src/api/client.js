...
export async function generateAssistantResponse(openaiMessages, modelName, parameters, openaiTools, callback, retryCount = 0) {
  ...
  try {
    ...
    // 所有错误（fetch、response、流处理）都在这里捕获
    logger.error('请求处理失败:', error.message);
    // 如果是流处理错误，记录失败
    if (error.message && !error.message.includes('切换token')) {
      tokenManager.recordFailure(token, { message: error.message });
    }
    throw error; // 重新抛出错误
  } finally {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (statusCode === 401 || statusCode === 403) {
        ...
        tokenManager.recordFailure(token, { statusCode, message: errorText, retryAfter: retryAfterMs });
        const error = new Error(`请求过于频繁(429)。错误详情: ${errorText}`);
        error.statusCode = statusCode;
        error.retryAfter = retryAfterMs ? Math.ceil(retryAfterMs / 1000) : null;
        throw error;
      } else if (statusCode >= 500 && statusCode < 600) {
        tokenManager.recordFailure(token, { statusCode, message: errorText });
        const error = new Error(`服务器错误(${statusCode})。错误详情: ${errorText}`);
        error.statusCode = statusCode;
        throw error;
      } else {
        tokenManager.recordFailure(token, { statusCode, message: errorText });
        const error = new Error(`获取模型列表失败 (${statusCode}): ${errorText}`);
        error.statusCode = statusCode;
        throw error;
      }
    }
